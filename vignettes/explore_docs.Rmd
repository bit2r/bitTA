...
vignette: >
  %\VignetteIndexEntry{Explore Docs}
...
데이터 분석을 위해서 데이터를 탐색하는 것(EDA, Exploratory Data Analysis)처럼 텍스트 데이터 분석 역시 데이터의 탐색이 필요합니다. 
...
### Text Data Explorer
 ...
 bitTA에서 제공하는 buzz 데이터를 탐색하기 위해서 다음과 같은 스크립트를 수행합니다.
...
검색한 패턴을 다른 문자열로 대체할 수도 있습니다. 텍스트 데이터의 정제를 위한 기능입니다. 다음은 `전업주부`라는 일반명사(결합명사)가 `전업 주부`로 분리되어 있는 사례를 보정하는 예시입니다.
...
`공동발생분석` 탭의 기능은 검색 키워드와 공동으로 발현하는 명사들을 추출해줍니다. 관심있는 키워드와 함께 이야기하는 주제를 파악하기 쉽습니다.
