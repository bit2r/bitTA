...
한글 텍스트 데이터에서 일반명사를 추출한 후 이것을 `tidy`한 데이터로 만들기 위해서는 `tidytext`의 `unnest_tokens()` 함수에 토크나이저로 `bitTA`의 `morpho_mecab()`를 사용합니다. 원하는 모습의 `tidy` 데이터가 만들어진 것입니닫.
...
`bitTA`의 `unnest_noun_ngrams()`는 추출된 n-grams 명사 토큰들을 tibble의 컬럼에 하나씩 붙여줍니다.
