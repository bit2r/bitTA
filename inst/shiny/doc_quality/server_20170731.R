...
shinyServer(function(input, output, session) {
  ...
  observeEvent(input$search, {
    ...
    output$ids <- renderText({
      if (is.null(cidx)) {
        .docData[, input$idname][idx[1]]
      } else {
        .docData[cidx, input$idname][idx[1]]
      }
    })
  ...
  observeEvent(input$down, {
    ...
    output$ids <- renderText({
      if (is.null(cidx)) {
        .docData[, input$idname][next_idx]
      } else {
        .docData[cidx, input$idname][next_idx]
      }
    })
  ...
  observeEvent(input$up, {
    ...
    output$ids <- renderText({
      if (is.null(cidx)) {
        .docData[, input$idname][prev_idx]
      } else {
        .docData[cidx, input$idname][prev_idx]
      }
    })
